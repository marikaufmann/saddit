...
const CreateCommunityForm = () => {
  ...
  const { mutate: createCommunity, isLoading } = useMutation({
    ...
    onError: (err) => {
      if (err instanceof AxiosError) {
        if (err.response?.status === 409) {
          return toast({
            title: "Subthreadit with this name already exists.",
    ...
    onSuccess: (data) => {
      ...
      router.refresh();
      return toast({
        description: `Subthreadit "${data}" was successfully created!`,
        variant: "default",
      });
    },
  });
  return (
    <div className="rounded-lg max-w-3xl mx-auto shadow-sm overflow-hidden w-full z-20">
      <div className="bg-[#f2f1ef] px-6 py-4 flex flex-col space-y-4 font-title">
      ...
      <div className="bg-[#edeae1] px-6 py-4">
